# ==============================================================================
# Metashape USGS Otomasyonu - v1.3.3 Stabil Safe Mode (v1.7.2 Uyumlu)
# DAA Mühendislik Bilişim - Deniz Aydınalp
# Güncelleme: 2025-12-09 | V1.7.2 Uyum Kontrolü ve Optimizasyon
# ==============================================================================
...
def log_message(message, level): 
    ...
    # Log seviyeleri (Information, Warning, Critical) doğrudan Metashape objesinden çağrılır.
...
def check_stop_criteria(prev_rmse):
    ...
    API uyumlu: m.residual.norm() kullanır.
    """
    ...
    for m in chunk.markers:
        # Sadece etkin GCP'leri kontrol et
        if m.reference.enabled:
            try:
                # KRİTİK: m.residual.norm() ile 3D kalıntı hatasını al
                error = m.residual.norm()
            except AttributeError:
                log_message("Hata: Marker objesi 'residual' niteliğine sahip değil. API hatası devam ediyor olabilir.", Metashape.Critical)
                return True 
    ...
    if num_markers == 0:
        log_message("Referansı etkin marker (GCP) bulunamadı.", Metashape.Warning)
        return False
...
def usgs_professional_workflow():
    ...
    chunk = Metashape.app.document.chunk
    log_message("--- DAA Mühendislik Fotogrametri USGS Workflow v1.3.3 Başladı (v1.7.2 Uyumlu) ---", Metashape.Information)
    ...
    log_message(f"Kamera Doğruluğu (XYZ) {CAMERA_ACCURACY_GCP_OVERRIDE}m olarak ayarlandı (GCP Güvencesi).", Metashape.Information)
    ...
    # Optimizasyon parametreleri: f, cx, cy, k1, k2, k3, p1, p2 (b1/b2 kapalı)
    ...
    while iter_count < max_iterations:
        ...
        log_message(f"İterasyon {iterasyon}: Max Reprojection Error = {max_reprojection_error:.4f} px", Metashape.Information)
